...
main(int argc, char *argv[])
{
    ...
    while(elapsedglobal< 2000) {
        ...
        if (elapsed >= MINTICKS) {
            ...
            start = end;
            j++;
            sum_opsw += (int) (opsw * MINTICKS / elapsed);
            sum_opsr += (int) (opsr * MINTICKS / elapsed);
            total_ops+=opsr+opsw;
            opsw = 0;
            opsr = 0;
    ...
    printf("Termino iobench %d: total ops %lu -->\t\n",pid, total_ops);
    printf("Promedio de OPW: %d\n", sum_opsw/j);
    printf("Promedio de OPR: %d\n", sum_opsr/j);
    pstat(pid);
    exit(0);
}
