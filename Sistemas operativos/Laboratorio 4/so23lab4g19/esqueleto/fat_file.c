...
// busca el último cluster válido de un file
static u32 fat_file_get_last_cluster(fat_file file) {
    u32 next_cluster = file->start_cluster, last_cluster;
    while(!fat_table_is_EOC(file->table, next_cluster)) {
        last_cluster = next_cluster;
        next_cluster = fat_table_get_next_cluster(file->table, next_cluster);
    }
    return last_cluster;
}
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    // Move cluster to last cluster readable/writable
    ...
    if (fat_table_is_EOC(file->table, cluster)) {
        printf("llegue al EOC\n");
        u32 new_cluster = fat_table_get_next_free_cluster(file->table);
        last_cluster = fat_file_get_last_cluster(file);
        printf("last_cluster: %u\n", last_cluster);
        fat_table_set_next_cluster(file->table, last_cluster, new_cluster);
    ...
    while (bytes_remaining > 0) {  // && !fat_table_is_EOC(file->table, cluster)
        ...
        cluster_off = fat_table_cluster_offset(file->table, cluster) +
                      fat_table_mask_offset(offset, file->table);
        printf("el offset del cluster %u es %lu\n", cluster, fat_table_cluster_offset(file->table, cluster));
        ...
        // si no pude escribir todo, creo un nuevo cluster
        printf("bytes_written_cluster %u: %lu\n", cluster, bytes_written_cluster);
        printf("bytes_to_write_cluster %u: %lu\n", cluster, bytes_to_write_cluster);
