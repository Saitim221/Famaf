...
int fat_volume_unmount(fat_volume vol) {
    ...
    // vuelvo a cambiar el nombre del LOGFILE para que este no sea ignorado al desmontar
    // lo busco en el file tree
    // file_node = fat_tree_node_search(vol->file_tree, LOG_FILE); 
    // file = fat_tree_get_file(file_node);
    // printf("%s\n", file->name);
    // if (file->dentry->base_name[0] == 0xE5) {    
    //     file->dentry->base_name[0] = LOG_FILE_BASENAME[0];
