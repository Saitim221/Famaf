...
int main(int argc, char *argv[]) {
    ...
    int minimun = minimun_temperature(array);

    printf("La temperatura minima es: %d\n", minimun);

    int max_temp_per_year[YEARS];

    maximun_temperature(array, max_temp_per_year);

    unsigned int month_with_more_rainfall_per_year[YEARS];

    months_with_rainfall(array, month_with_more_rainfall_per_year);

    for(unsigned int year = 0; year < YEARS; year++){
        printf("La mayor temperatura del ano %d es: %d\n", year+1980, max_temp_per_year[year] );
        printf("El mes con mas precipitaciones del ano %d es %u\n", year+1980, month_with_more_rainfall_per_year[year]);
    }
