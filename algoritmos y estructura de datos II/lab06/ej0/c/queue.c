...
queue queue_disscard(queue q, unsigned int n){
    assert(invrep(q) && !queue_is_empty(q) && queue_size(q) > n);
        ...
        if(n == 0){
            ...
            struct s_node *killme = q->first;
            struct s_node  *killmeto = killme->next;
            while (n != 1)
            {
                killme = killme->next;
                killmeto = killmeto->next;
                n--;
            }
            killme->next = killmeto->next;
            killmeto = destroy_node(killmeto);
            killme = NULL;
            //Uso dos punteros que van avanzando pocisiones, no se puede hacer con uno solo
