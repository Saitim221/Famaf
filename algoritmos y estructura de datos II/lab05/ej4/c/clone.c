...
int main(void) {
    ...
    copy = string_clone(original, sizeof(original)*strlen(original));
    printf("Original: %s\n", original);
    copy[0] = 'A';
    copy[1] = ' ';
    copy[2] = 'l';
...
/*EL problema esta en copy = string_clone(original, sizeof(original)/(sizeof(char) - 1));
sizeof(original) ya es un caracter, lo que estabamos haciendo ahi no tenia sentido, pues estabamos intentando asignar una memoria tamano char/char-1
lo que hice fue calcular el largo mejor y asi teniamos cuanto espacio realmente necesitaba.
