...
def mock_response():
    with requests_mock.Mocker() as m:
        ...
        m.get('http://localhost:5000/peliculas/I',status_code=200)

        m.get('http://localhost:5000/peliculas/genero/Drama',json=[
            {'id': 1, 'titulo': 'Indiana Jones', 'genero': 'Drama'},
            {'id': 2, 'titulo': 'Star Wars', 'genero': 'Drama'}]
        )

        m.get('http://localhost:5000/peliculas/random',status_code=200,json=[
            {'id': 1, 'titulo': 'Indiana Jones', 'genero': 'Acción'},
            ])

        m.get('http://localhost:5000/peliculas/random/Drama',status_code=200,json=[
            {'id': 1, 'titulo': 'Indiana Jones', 'genero': 'Drama'}
            ])
        m.get('http://localhost:5000/peliculas/sugerencia/Drama',status_code=200,json=[
            {
        "dia": 29,
        "genero": "Drama",
        "id": 12,
